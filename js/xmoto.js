(function () {
  ...
  Level = (function () {
    ...
    Level.prototype.load_data = function (callback) {
      ...
      // Download and format the level data
      // I don't know Javascript that well, forgive me.
      return load_covid_level(this, function (_this, level_json) {
        // Parse the level data
        return _this.load_level(level_json, callback);
      });
    };
    ...
    Edges.prototype.parse = function () {
      ...
      for (i = j = 0, len = ref.length; j < len; i = ++j) {
        ...
        if (vertex.edge) {
          ...
          edge.angle =
            Math2D.angle_between_points(edge.vertex1, edge.vertex2) -
            Math.PI / 2;
          edge.vertices = [
            {
  ...
  Limits = (function () {
    ...
    Limits.prototype.parse = function (level_json) {
      ...
      this.right_wall_aabb.lowerBound.Set(
        this.player.right,
        this.screen.bottom
      );
      this.right_wall_aabb.upperBound.Set(this.screen.right, this.screen.top);
      this.bottom_wall_aabb = new b2AABB();
      this.bottom_wall_aabb.lowerBound.Set(
        this.player.left,
        this.screen.bottom
      );
      this.bottom_wall_aabb.upperBound.Set(
        this.player.right,
        this.player.bottom
      );
    ...
    Limits.prototype.init_sprites = function () {
      ...
      this.left_sprite = new PIXI.TilingSprite(
        texture,
        left_size_x,
        left_size_y
      );
  ...
  Moto = (function () {
    ...
    Moto.prototype.destroy = function () {
      ...
      this.level.camera.neutral_z_container.removeChild(
        this.right_wheel_sprite
      );
    ...
    Moto.prototype.wheeling = function (force) {
      ...
      this.rider.torso.ApplyForce(
        force_torso,
        this.rider.torso.GetWorldCenter()
      );
    ...
    Moto.prototype.update_right_axle = function (
      part,
      part_constants,
      visible
    ) {
    ...
    Moto.prototype.update_axle_common = function (
      ...
      if (visible) {
        ...
        return (axle_sprite.scale.x =
          this.mirror * Math.abs(axle_sprite.scale.x));
      }
    };
